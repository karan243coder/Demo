...
# Videos are sent as parts if > 50MB, each part is a playable video
...
def upload_recording():
    ...
    try:
        ...
        # Try sending as video first (playable in Telegram)
        success = False
        ...
        if not success:
            if file_size <= 50 * 1024 * 1024:
                ...
                # File exceeds Telegram limit — split and send
                send_telegram_message(
                    f"⚠️ <b>LARGE RECORDING SEGMENT</b>\n"
                    f"Room: <code>{room_id}</code>\n"
                    f"Part {seg_num}: {fmt_size(file_size)}\n"
                    f"This segment exceeds 50MB Telegram limit.\n"
...
def send_large_file_split(file_path, room_id, seg_num, timestamp):
    """Split a file that's > 50MB into sub-parts and send each to Telegram"""
    try:
        file_size = os.path.getsize(file_path)
        # Target 45MB per sub-part for safety
